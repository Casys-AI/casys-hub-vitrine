...
<Aside type="caution">
  从中间件返回 `{ error: true, message }` 会序列化为**普通**工具结果，而非 MCP `isError` 响应。要发出工具错误信号，应抛出 `Error`（或让其传播）— 框架的 `toolErrorMapper` 会将其转换为正确的 MCP 错误格式。
</Aside>
...
- [McpApp API](/zh/docs/api/concurrent-mcp-server/) — `use()` 和 `registerTool()` 完整参考
