...
Higher priority wins. Ship sane defaults in YAML and override specific values per environment with env vars.
...
Every YAML auth field can be overridden with `MCP_AUTH_*` environment variables:
