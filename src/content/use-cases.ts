...
這個客戶的挑戰凝練了 PML 旨在解決的問題。導航相互依賴的知識圖譜、追蹤決策路徑、測試參數組合——這正是 <strong>MCP Engine</strong> 在規模化層面的目標。當前的研發方向：一個代理將接收招標書，像產品 BOM 一樣分解依賴關係，探索圖譜路徑，並提出最優組合——每一步都<strong>可追蹤、確定性的</strong>。這一願景正在積極建設中，借鑑了產品生命週期管理和基於模型的系統工程原則。`,
  ...
  },
  {
    ...
    icon: 'dashboard',
    title: {
      en: 'A concrete MCP workflow on top of ERPNext',
      fr: 'Un workflow MCP concret sur ERPNext',
      zh: '基于 ERPNext 的具体 MCP 工作流',
      'zh-TW': '基於 ERPNext 的具體 MCP 工作流',
    },
    situation: {
      en: `Most AI demos show a chat prompt returning text. MCP ERPNext goes further: it turns a <strong>real ERP system</strong> into an inspectable, interactive MCP app surface. The agent doesn't just answer questions — it drives a Kanban workflow, triggers business actions, and exposes every execution step for traceability.`,
      fr: `La plupart des démos IA montrent un prompt chat retournant du texte. MCP ERPNext va plus loin : il transforme un <strong>vrai système ERP</strong> en surface MCP inspectable et interactive. L'agent ne se contente pas de répondre aux questions — il pilote un workflow Kanban, déclenche des actions métier et expose chaque étape d'exécution pour la traçabilité.`,
      zh: `大多数 AI 演示展示的是聊天提示返回文本。MCP ERPNext 更进一步：它将一个<strong>真实的 ERP 系统</strong>转变为可检查的交互式 MCP 应用界面。代理不仅仅回答问题——它驱动看板工作流，触发业务操作，并暴露每个执行步骤以实现可追溯性。`,
      'zh-TW': `大多數 AI 演示展示的是聊天提示返回文本。MCP ERPNext 更進一步：它將一個<strong>真實的 ERP 系統</strong>轉變為可檢查的互動式 MCP 應用介面。代理不僅僅回答問題——它驅動看板工作流，觸發業務操作，並暴露每個執行步驟以實現可追溯性。`,
    },
    friction: {
      en: `Traditional ERP integrations require custom development for each automation. Business users must navigate complex UIs to perform routine operations. There's no visibility into what the system is doing or why.`,
      fr: `Les intégrations ERP traditionnelles nécessitent un développement spécifique pour chaque automatisation. Les utilisateurs métier doivent naviguer des interfaces complexes pour des opérations de routine. Aucune visibilité sur ce que fait le système ou pourquoi.`,
      zh: `传统 ERP 集成需要为每个自动化进行定制开发。业务用户必须导航复杂的界面才能执行常规操作。无法看到系统在做什么或为什么。`,
      'zh-TW': `傳統 ERP 整合需要為每個自動化進行定制開發。業務用戶必須導航複雜的介面才能執行常規操作。無法看到系統在做什麼或為什麼。`,
    },
    shift: {
      en: `With MCP ERPNext, the ERP becomes an <strong>agent-driven surface</strong>. The AI composes interactive UI components (Kanban boards, forms, charts) on the fly, connected to real ERPNext data. Every tool call is traced, every action is observable. It's the same stack that powers the other use cases — applied to a real business system.`,
      fr: `Avec MCP ERPNext, l'ERP devient une <strong>surface pilotée par agent</strong>. L'IA compose des composants d'interface interactifs (tableaux Kanban, formulaires, graphiques) à la volée, connectés aux données réelles d'ERPNext. Chaque appel d'outil est tracé, chaque action est observable. C'est la même stack que les autres cas d'usage — appliquée à un vrai système métier.`,
      zh: `借助 MCP ERPNext，ERP 成为<strong>代理驱动的界面</strong>。AI 动态组合交互式 UI 组件（看板、表单、图表），连接到真实的 ERPNext 数据。每个工具调用都被追踪，每个操作都是可观测的。这是驱动其他用例的同一技术栈——应用于真实的业务系统。`,
      'zh-TW': `借助 MCP ERPNext，ERP 成為<strong>代理驅動的介面</strong>。AI 動態組合互動式 UI 組件（看板、表單、圖表），連接到真實的 ERPNext 資料。每個工具調用都被追蹤，每個操作都是可觀測的。這是驅動其他用例的同一技術棧——應用於真實的業務系統。`,
    },
    punchline: {
      en: 'Instead of imagining the future, inspect a workflow that already works.',
      fr: "Au lieu d'imaginer le futur, inspectez un workflow qui fonctionne déjà.",
      zh: '不要想象未来，检查一个已经在运行的工作流。',
      'zh-TW': '不要想像未來，檢查一個已經在運行的工作流。',
    },
    stack: ['@casys/mcp-server', '@casys/mcp-std', '@casys/mcp-bridge', 'MCP Apps', 'PML'],
  },
  {
    id: 'mcp-einvoice',
    tag: 'MCP E-Invoice',
    icon: 'receipt_long',
    title: {
      en: 'French e-invoicing, wired into your agent stack',
      fr: 'La facturation électronique française, branchée sur votre stack agent',
      zh: '法国电子发票，接入您的代理技术栈',
      'zh-TW': '法國電子發票，接入您的代理技術棧',
    },
    situation: {
      en: `French e-invoicing reform (2026) requires every business to emit, receive, and report invoices electronically. MCP E-Invoice provides <strong>27 MCP tools</strong> covering the full invoice lifecycle: emission, search, download, status tracking, directory lookup, and DGFiP e-reporting.`,
      fr: `La réforme de la facturation électronique française (2026) impose à chaque entreprise d'émettre, recevoir et déclarer ses factures électroniquement. MCP E-Invoice fournit <strong>27 outils MCP</strong> couvrant tout le cycle de vie facture : émission, recherche, téléchargement, suivi de statut, consultation annuaire et e-reporting DGFiP.`,
      zh: `法国电子发票改革（2026年）要求每家企业以电子方式发出、接收和申报发票。MCP E-Invoice 提供 <strong>27 个 MCP 工具</strong>，涵盖完整的发票生命周期：发出、搜索、下载、状态追踪、目录查询和 DGFiP 电子申报。`,
      'zh-TW': `法國電子發票改革（2026年）要求每家企業以電子方式發出、接收和申報發票。MCP E-Invoice 提供 <strong>27 個 MCP 工具</strong>，涵蓋完整的發票生命週期：發出、搜索、下載、狀態追蹤、目錄查詢和 DGFiP 電子申報。`,
    },
    friction: {
      en: `E-invoicing compliance is complex: multiple formats (CII, UBL, Factur-X), certified platforms (PA) with different APIs, and a lifecycle with 15+ status transitions. Most solutions lock you into a single vendor.`,
      fr: `La conformité facture électronique est complexe : multiples formats (CII, UBL, Factur-X), plateformes agréées (PA) avec des API différentes, et un cycle de vie avec 15+ transitions de statut. La plupart des solutions vous enferment chez un seul éditeur.`,
      zh: `电子发票合规性很复杂：多种格式（CII、UBL、Factur-X），认证平台（PA）有不同的 API，生命周期有 15+ 种状态转换。大多数解决方案将您锁定在单一供应商。`,
      'zh-TW': `電子發票合規性很複雜：多種格式（CII、UBL、Factur-X），認證平台（PA）有不同的 API，生命週期有 15+ 種狀態轉換。大多數解決方案將您鎖定在單一供應商。`,
    },
    shift: {
      en: `MCP E-Invoice uses a <strong>PA-agnostic adapter pattern</strong>. Switch between certified platforms (Iopole, etc.) without changing your agent code. 4 built-in MCP Apps viewers let you inspect invoices, track status, browse directories, and manage documents — all within your agent conversation.`,
      fr: `MCP E-Invoice utilise un <strong>pattern d'adaptateur agnostique PA</strong>. Changez de plateforme agréée (Iopole, etc.) sans modifier le code de vos agents. 4 viewers MCP Apps intégrés pour inspecter les factures, suivre les statuts, parcourir les annuaires et gérer les documents — le tout dans votre conversation agent.`,
      zh: `MCP E-Invoice 使用<strong>平台无关的适配器模式</strong>。在认证平台（Iopole 等）之间切换，无需更改代理代码。4 个内置 MCP Apps 查看器让您检查发票、追踪状态、浏览目录和管理文档——全部在代理对话中完成。`,
      'zh-TW': `MCP E-Invoice 使用<strong>平台無關的適配器模式</strong>。在認證平台（Iopole 等）之間切換，無需更改代理代碼。4 個內建 MCP Apps 檢視器讓您檢查發票、追蹤狀態、瀏覽目錄和管理文檔——全部在代理對話中完成。`,
    },
    punchline: {
      en: '27 tools, PA-agnostic, observable — e-invoicing as an agent capability.',
      fr: '27 outils, agnostique PA, observable — la facturation électronique comme capacité agent.',
      zh: '27 个工具，平台无关，可观测——电子发票作为代理能力。',
      'zh-TW': '27 個工具，平台無關，可觀測——電子發票作為代理能力。',
    },
    stack: ['@casys/mcp-server', '@casys/mcp-compose', 'OAuth2', 'MCP Apps', 'Iopole Adapter'],
  },
];
